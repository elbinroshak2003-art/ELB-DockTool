...
public sealed class MainForm : Form
{
    ...
    PdbFixerStatus pdbFixerStatus = PdbFixerStatus.NotInstalled("Status has not been checked yet.");
    ...
    Panel BuildDashboard()
    {
        ...
        var text = new Label { Text = "1   Structure source (local PDB/mmCIF or PDB ID)\n2   PDBFixer repair (optional)\n3   Receptor Structure Inspector\n4   Remove crystallographic water\n5   Remove non-protein HETATM\n6   Polar H (optional) → Meeko → receptor PDBQT\n7   Define docking box → AutoDock Vina → review results", AutoSize = true, ForeColor = TextColor, Font = new Font("Segoe UI", 10.5F), Location = new Point(22, 62) };
    ...
    Panel BuildPhytochemicalLibrary()
    {
        ...
        // Keep actions in a clear workflow order: inspect/send one → download/open → send all.
        var props = MakeButton("PROPERTIES", 130, 36, Panel2, TextColor); props.Location = new Point(22, 74);
        var send = MakeButton("SEND TO LIGANDS (PDBQT)", 205, 36, Purple, Color.White); send.Location = new Point(162, 74);

        var sdf = MakeButton("DOWNLOAD SDF", 155, 36, Panel2, TextColor); sdf.Location = new Point(22, 118);
        var pdbqt = MakeButton("DOWNLOAD PDBQT", 165, 36, Panel2, TextColor); pdbqt.Location = new Point(187, 118);
        var openFolder = MakeButton("OPEN LIBRARY FOLDER", 190, 36, Panel2, TextColor); openFolder.Location = new Point(362, 118);

        var sendAllPdbqt = MakeButton("SEND ALL AS PDBQT", 175, 36, Purple, Color.White); sendAllPdbqt.Location = new Point(22, 162);
        var sendAllSdf = MakeButton("SEND ALL AS SDF", 165, 36, Purple, Color.White); sendAllSdf.Location = new Point(207, 162);
        var stopSendAll = MakeButton("STOP SEND ALL", 145, 36, Panel2, TextColor); stopSendAll.Location = new Point(382, 162);
        stopSendAll.Enabled = false;
        var actionStatus = MutedLabel("Select a compound above."); actionStatus.Location = new Point(545, 170);
        ...
        {
            props, send, sdf, pdbqt, openFolder, sendAllPdbqt, sendAllSdf, stopSendAll, actionStatus
        });
    ...
    Panel BuildDocking()
    {
        ...
        var selectAllInspector = MakeButton("SELECT ALL", 150, 34, Panel2, TextColor); selectAllInspector.Location = new Point(12, 568);
        var deleteSelected = MakeButton("DELETE SELECTED", 150, 34, Color.FromArgb(92, 34, 45), Color.White); deleteSelected.Location = new Point(170, 568);
        var restoreOriginal = MakeButton("RESTORE ORIGINAL", 150, 34, Panel2, TextColor); restoreOriginal.Location = new Point(12, 608);
        inspector.Controls.Add(deleteSelected); inspector.Controls.Add(restoreOriginal); inspector.Controls.Add(selectAllInspector);
        ...
        var enablePdbFixer = new CheckBox { Text = "Enable PDBFixer repair", Checked = false, AutoSize = true, ForeColor = TextColor, Location = new Point(12, 24) };
        ...
        var addMissingResidues = new CheckBox { Text = "Add missing residues", Checked = false, AutoSize = true, ForeColor = TextColor, Location = new Point(222, 51) };
        ...
        var addPdbFixerHydrogens = new CheckBox { Text = "Add missing hydrogens", Checked = false, AutoSize = true, ForeColor = TextColor, Location = new Point(12, 78) };
        ...
        var pdbFixerPh = new NumericUpDown { Location = new Point(290, 76), Size = new Size(78, 26), DecimalPlaces = 1, Minimum = 0, Maximum = 14, Increment = 0.5M, Value = 7.0M, BackColor = Color.FromArgb(9, 15, 25), ForeColor = TextColor };
        ...
        var addPolarHydrogens = new CheckBox { Text = "Add polar hydrogens (Open Babel)", Checked = true, AutoSize = true, ForeColor = TextColor, Location = new Point(22, 381) };
        ...
        var recLoad = MakeButton("LOAD SAVED", 120, 38, Panel2, TextColor);
        recLoad.Location = new Point(345, 246);
        recLoad.Click += (_, _) => AddStoredFiles(recList, ReceptorsFolder(), "receptor");
        ...
        receptor.Controls.Add(recList); receptor.Controls.Add(recAdd); receptor.Controls.Add(recClear); receptor.Controls.Add(recLoad); receptor.Controls.Add(recSave); receptor.Controls.Add(gridEdit);
        ...
        var ligLoadSaved = MakeButton("LOAD SAVED", 140, 38, Panel2, TextColor);
        ligLoadSaved.Location = new Point(350, 334);
        ligLoadSaved.Click += (_, _) => LoadSavedLigandPanel(ligList);
        ligand.Controls.AddRange(new Control[] { ligList, ligAdd, ligClear, ligClearAll, ligLoad, ligAuto, ligMin, ligFolder, ligSmiles, ligSave, ligLoadSaved });
...
");
        ...
        status.Text = "PDBFixer: repairing the current working copy...";
        status.ForeColor = Color.Gold;
        var result = await RunProcess(pdbFixerStatus.Python, $"{Q(script)} {Q(inputPath)} {Q(output)} {flags} {ph.ToString(CultureInfo.InvariantCulture)}", AppServices.BaseDir);
        File.WriteAllText(Path.Combine(runDir, "pdbfixer_repair.log"), $"Input: {inputPath}\r\nOutput: {output}\r\nAdd heavy atoms: {addHeavyAtoms}\r\nAdd residues: {addResidues}\r\nReplace nonstandard residues: {replaceNonstandard}\r\nAdd hydrogens: {addHydrogens}\r\npH: {ph.ToString(CultureInfo.InvariantCulture)}\r\n\r\n{result.Stdout}\r\n{result.Stderr}");
    ...
    async Task<string?> PrepareReceptorAsync(string inputPath, bool usePdbFixer, bool addHeavyAtoms, bool addMissingResidues, bool replaceNonstandard, bool addPdbFixerHydrogens, decimal pdbFixerPh, bool removeWater, bool removeNonProteinHet, bool addPolarHydrogens, bool deleteBadResidues, Label status)
    {
        ...
        try
        {
            ...
            // For PDB/ENT input, always create a preparation-specific copy. This
            ...
            // For PDB/ENT input, optionally add only polar hydrogens with Open Babel
            // to a temporary preparation copy before Meeko. The original input is
    ...
    Panel BuildResults()
    {
        ...
        card.Height = 690;
        var actions = new FlowLayoutPanel { Location = new Point(22, 86), Size = new Size(1120, 50), AutoSize = false, WrapContents = false, FlowDirection = FlowDirection.LeftToRight, BackColor = Color.Transparent, Padding = new Padding(0), Margin = new Padding(0) };
    ...
    Panel BuildSettings()
    {
        ...
        var detect = MakeButton("DETECT OPEN BABEL", 190, 42, Panel2, TextColor);
        detect.Location = new Point(22, 512);
        detect.Click += (_, _) =>
        {
            var detected = AppServices.DetectExecutable("obabel.exe");
        ...
        var save = MakeButton("SAVE SETTINGS", 190, 42, Purple, Color.White);
        save.Location = new Point(224, 512);
        save.Click += (_, _) =>
        {
        ...
        reset.Location = new Point(426, 512);
        reset.Click += (_, _) =>
        {
        ...
        var verifyMeeko = MakeButton("VERIFY PYTHON / MEEKO", 210, 42, Panel2, TextColor);
        verifyMeeko.Location = new Point(590, 512);
        ...
        card.Controls.AddRange(new Control[] { detect, save, reset, verifyMeeko });
        p.Controls.Add(card);
        ...
        var pythonValue = new Label { Location = new Point(160, 106), Size = new Size(670, 24), ForeColor = TextColor, Text = string.IsNullOrWhiteSpace(pdbFixerStatus.Python) ? "Detecting…" : pdbFixerStatus.Python + "  " + pdbFixerStatus.PythonVersion };
        var versionsTitle = L("Versions", 22, 144);
        var versionsValue = new Label { Location = new Point(160, 142), Size = new Size(670, 24), ForeColor = TextColor, Text = pdbFixerStatus.IsUsable ? $"PDBFixer {pdbFixerStatus.PdbFixerVersion}   •   OpenMM {pdbFixerStatus.OpenMmVersion}" : "PDBFixer/OpenMM not detected" };
